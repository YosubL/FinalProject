...
<style>
	...
	#mycontent > form:nth-child(5) > span{
		font-size:15pt;
		margin-right:50px;
		position:relative;
		left:30px;
...
<script>
	...
	$(document).ready(function(){
		...
		$("button#btnSearch").click(function(){
	        ...
	        frm.action = "<%= ctxPath%>/CelebratePayStatus.gw?app_date=" + app_date + "&deptName=" + deptName + "&title=" + title + "&gender=" + gender;
	        frm.submit();
	    });
		...
		$("button#btnSearch2").click(function(){
	    	const frm = document.searchFrm;
	        const name = $("#name").val();
	        frm.action = "<%= ctxPath%>/CelebratePayStatus.gw?name=" + name;
	        frm.submit();
	    });
		...
		// 초기화 버튼 클릭 시 폼 초기화
   	    $("button#btnReset").click(function() {
   	      ...
   	      frm.action = "<%= ctxPath %>/CelebratePayStatus.gw"; // 폼 액션 초기화
   	      frm.submit();
   	    });
		...
		$("input#approved").click(function(){
				...
				if(gradelevel < 10) {
					...
					var vac_approved = confirm("승인 하시겠습니까?");
					...
					if(vac_approved) {
						...
						frm.method = "post";
						frm.action = "<%= ctxPath %>/CelebrateStatus_Update.gw";
						frm.submit();
					}
				}
			}); //end of $("input#approved").click(function(){----------------------
	
		//반려하기 버튼 		
		$("input#rejected").click(function(){
			...
			if(gradelevel < 10) {
				...
				var vac_approved = confirm("반려 하시겠습니까?");
				...
				if(vac_approved) {
					...
					frm.method = "post";
					frm.action = "<%= ctxPath %>/CelebrateStatus_reject.gw";
					frm.submit();
				}
			}
		});//end of $("input#rejected").click(function(){-------------------------
	    ...
	    <form name="vac_manage_frm">
			<div class='m-4' style="margin: 7% 0% 5% 0%; width: 95%;">
				...
				<table class="table table-bordered table-sm ">
					<thead>
						<tr>
							<c:if test="${not empty requestScope.CelebrateStatusList && sessionScope.loginuser.gradelevel == 10}">
			               		<th class='col col-1'><input type="checkbox" id="selectAll" class="mr-1"/>전체선택</th>
			             	</c:if>
							<th>No</th>
							<th>신청번호</th>
							<th>사원번호</th>
					...
					<tbody>
					    <c:if test="${not empty requestScope.CelebrateStatusList}">
					        <c:forEach var="StatusList" items="${requestScope.CelebrateStatusList}" varStatus="status" > 
			...
			<div align="center">
				<c:if test="${empty requestScope.CelebrateStatusList}">
					 신청내역이 존재하지 않습니다.
				</c:if>
			</div>
			...
			<c:if test="${not empty requestScope.CelebrateStatusList}">
				<div align="right" style="margin: 3% 4% 3% 0;">
					...
					<input type="submit" id="rejected" name="rejected" value="반려" class="ml-4"/>
				</div>
			</c:if>
		</form>	 
